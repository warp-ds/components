		...
		(day: Date) => {
			if (startDate && !endDate) {
				const firstAllowedEndDate =
					startDate && addDays(startDate, minimumNights);

				// Disable all days that are between the start and first allowed end date.
				if (
					utils.isAfterDay(day, startDate) &&
					utils.isBeforeDay(day, firstAllowedEndDate)
				) {
					return true;
				}
			}
	...
	return function reducer(
		...
		switch (event.type) {
			case "FOCUS":
				...
				return event.input.current === endInputRef.current
					? INTERACTING_END_DATE
					: INTERACTING_START_DATE;
...
function onDaySelect({
	...
	if (state === INTERACTING_START_DATE) {
		...
		const isStartDateAfterEndDate =
			utils.isBeforeDay(lastAllowedStartDate, day) ||
			utils.isAfterDay(startDate, endDate);
